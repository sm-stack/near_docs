...
> 네트워크의 활성 밸리데이터에 쿼리하여 세부 정보 및 블록체인의 검증 상태를 반환합니다.

- 메서드: `validators`
- 매개변수: `["block hash"]`, `[block number]`, 또는 최신 블록에 대해서는 `[null]`

**참고:** `["block hash"]`와 `[block number]`의 경우, 에포크의 마지막 블록에서 쿼리해야 합니다.
