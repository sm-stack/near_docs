...
<table class="custom-stripe">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>공개 키가 생성되지 않았거나 이미 삭제되었기 때문에 요청된 <code>public_key</code>를 찾을 수 없습니다.has not been found while viewing since the public key has not been created or has been already deleted</td>
      <td>
        <ul>
...
> 계정에 배포된 계약 코드(Wasm 바이너리)를 반환합니다. 반환된 코드는 base64로 인코딩됩니다.
